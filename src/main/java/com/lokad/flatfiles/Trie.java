...
/**
 * A trie data structure for matching cell contents with unique and sequential
 * integer identifiers.
 * 
 * 
 * This class is intended for maximum performance. The implementation, beyond
 * being fine-tuned, follows a simple design constraint: -> The system should
 * perform (N + 2 log N) memory allocations, where N is the number of different
 * byte sequences stored.
 * 
 * The trie uses compressed sequences. That is, to store sequences ABC and ABD,
 * only three nodes are used: [AB], [C] and [D] ; this is different from an
 * uncompressed version where each node would hold exactly one byte: [A], [B],
 * [C] and [D].
 * 
 * Each trie node contains the following fields: -> 'Buffer' is a pointer to a
 * buffer where the compressed sequence is held. -> 'Start' and 'End' are the
 * indices, within 'Buffer', where the compressed sequence is stored. This
 * allows sharing buffers between nodes. -> 'First' is the first four bytes of
 * the compressed sequence. -> 'Reference', if > 0, is the unique identifier of
 * the prefix ending at that node. -> 'Children' is an array of children of the
 * node, of size <see cref="HashSizeAtLength"/> of the prefix length at that
 * node. The starting byte of each child, mod the size, is the index of that
 * child within the array. -> 'NextSibling' is an implementation of a
 * list-of-children in the case where several children end up in the same cell
 * of their parent's array.
 * 
 * Those fields are all integers (or arrays of integers) and are not represented
 * by a class/struct: instead, they are implicitely represented as cells within
 * an array of integers.
 * 
 * 'nodeI' (note the final I) is the index of the first cell in a node. The
 * 'End' field can be found at `_nodes[nodeI + End]`, and so on.
 * 
 * 'nodeR' is the index of a cell holding a 'nodeI'. This is used by the
 * algorithm when inserting new nodes, because nodeR is the index of the cell
 * referencing the node (and is the cell which has to be changed).
 * 
 * Hungarian: Values 'iXXX' represent the 'input' source data Values 'bXXX'
 * represent the 'buffer' trie data
 */
public final class Trie
{
	...
	/**
	 * Individual values indexed by their unique identifier, as generated by the
	 * trie.
	 * 
	 * 
	...
	/**
	 * Computes the size of the "Children" hashtable based on the length of the
	 * prefix at that node.
	 * 
	 * 
	 * Hash table size decreases exponentially, reaching '1' at length 8. This
	 * means that shorter strings use up more memory to avoid 'NextSibling'
	 * traversal, while longer strings use 'NextSibling' traversal to lower
	 * memory usage.
	 */
	...
	/**
	 * Reads bytes iBytes[iStart .. iEnd] and returns an unique identifier k for
	 * that sequence, such that <code>trie.Values[k]</code> is the corresponding
	 * sequence.
	 */
	...
	/**
	 * Create a new node as a child of 'nodeI' and containing
	 * 'iBytes[iPos..iEnd)' and pointing to the appropriate new buffer.
	 * 
	 * 
	 * The new child has _nodes[childR] as its next sibling, and becomes the
	 * first child of its parent in cell childR.
	 */
